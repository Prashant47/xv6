...
bfree(int dev, uint b)
{
  ...
  m = 1 << (bi % 8);
  if((bp->data[bi/8] & m) == 0){
    cprintf("while freeing value of b is %d\n",b);
...
//      cprintf("allocating block for 1st single indirect %d\n",addr);
    }
    bp = bread(ip->dev, addr);
    a = (uint*)bp->data;
    if((addr = a[bn]) == 0){
      a[bn] = addr = balloc(ip->dev);
 //     cprintf("allocating block for 2nd single indirect %d\n",addr);
      log_write(bp);
    }
    brelse(bp);
    return addr;
...
//*part being added
// this is for double indirect 


  bn -= NINDIRECT;
//  cprintf("printnig inode\n");
//  for (i = 0 ; i < NDIRECT+2 && flagforme;i++)
//    cprintf("a[%d] = %d\n",i,ip->addrs[i]);
...
//    cprintf("1st addr is %d\n", addr);
    bp = bread(ip->dev, addr);
    a = (uint*)bp->data;
//  cprintf("printing value of a \n");
...
//	cprintf("a[%2d] = %d",a[i]);
 ...
 //       cprintf("allocating block for 3rd double indirect %d \n", addr);
	log_write(bp);
    }
    brelse(bp);
    return addr;
